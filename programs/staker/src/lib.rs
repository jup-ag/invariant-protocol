...
pub mod staker {
    ...
    pub fn stake(ctx: Context<CreateUserStake>, _index: u32) -> Result<()> {
        instructions::stake::handler(ctx)
    }

    pub fn withdraw(ctx: Context<Withdraw>, _index: u32, nonce: u8) -> Result<()> {
    ...
    pub fn close_stake_by_owner(ctx: Context<CloseStakeByOwner>, _index: u32) -> Result<()> {
